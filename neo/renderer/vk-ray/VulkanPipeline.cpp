...
#include "renderer/vk-ray/VulkanPipeline.h"
...
bool idVulkanRayPipelineManager::Init( VkDevice device_, VkRenderPass renderPass,
...
VkPipeline idVulkanRayPipelineManager::GetPipeline( vulkanPipelineType_t type ) const {
	return pipelines[type].pipeline;
}

VkPipelineLayout idVulkanRayPipelineManager::GetPipelineLayout( vulkanPipelineType_t type ) const {
	return pipelines[type].layout;
}

VkDescriptorPool idVulkanRayPipelineManager::CreateDescriptorPool( uint32_t maxSets ) const {
...
VkShaderModule idVulkanRayPipelineManager::LoadShaderModule( const uint32_t *code, uint32_t size, const char *name ) {
	if ( code == NULL || size == 0 ) {
...
bool idVulkanRayPipelineManager::CreateDescriptorSetLayouts() {
...
bool idVulkanRayPipelineManager::CreatePipelineLayouts() {
...
bool idVulkanRayPipelineManager::CreatePipelines( VkRenderPass renderPass,
	VkExtent2D extent, const char *shaderDir )
{
	struct shaderPair_t {
...
VkPipelineLayout idVulkanRayPipelineManager::GetShaderPassLayout() const {
	// Shader pass pipelines reuse the gui pipeline layout
	return pipelines[VK_PIPELINE_GUI].layout;
}

VkPipeline idVulkanRayPipelineManager::GetShaderPassPipeline( uint64_t glState, int cullMode ) {
